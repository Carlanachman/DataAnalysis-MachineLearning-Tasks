...
def efficient_gradient_descent(X, y, theta, alpha, num_iters):
    ...
    for _ in range(num_iters):
        ...
        theta -= alpha * gradients  # update param

        cost = compute_cost(X, y, theta) # compute new cost
    ...
    for i in range(5):
        ...
        for feature in range(X_train.shape[1]):
            if feature not in selected_features:
                # Create a new feature set with the current feature added
                ...
                theta = np.zeros(X_train_subset.shape[1])

                theta, _ = efficient_gradient_descent(X_train_subset, y_train, theta, best_alpha, iterations)
                validation_loss = compute_cost(X_val_subset, y_val, theta)
...
def create_square_features(df):
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            col2 = columns[j]
            new_col = f"{col1}*{col2}"
            df_poly[new_col] = df[col1] * df[col2]
